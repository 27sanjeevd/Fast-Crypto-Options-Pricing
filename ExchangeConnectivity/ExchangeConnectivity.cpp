...
void ExchangeWebsocketClient::start() {
    ...
    resolver_.async_resolve(HOST, PORT, beast::bind_front_handler(&ExchangeWebsocketClient::on_resolve, this));
...
void ExchangeWebsocketClient::on_resolve(beast::error_code ec, tcp::resolver::results_type results) {
    if (ec) {
    ...
    beast::get_lowest_layer(*ws_).async_connect(results, beast::bind_front_handler(&ExchangeWebsocketClient::on_connect, this));
}

void ExchangeWebsocketClient::on_connect(beast::error_code ec, tcp::resolver::results_type::endpoint_type ep) {
    if (ec) {
    ...
    beast::get_lowest_layer(*ws_).expires_after(CONNECTION_TIMEOUT);
    ws_->next_layer().async_handshake(ssl::stream_base::client, beast::bind_front_handler(&ExchangeWebsocketClient::on_ssl_handshake, this));
}
...
void ExchangeWebsocketClient::on_ssl_handshake(beast::error_code ec) {
    ...
    ws_->async_handshake(HOST, TARGET, beast::bind_front_handler(&ExchangeWebsocketClient::on_handshake, this));
}
...
void ExchangeWebsocketClient::on_handshake(beast::error_code ec) {
    ...
    // Subscribe to BTCUSD perpetual orderbook
    std::string subscription = R"({"id":1,"method":"subscribe","params":{"channels":["book.BTCUSD-PERP.10"],"book_subscription_type":"SNAPSHOT_AND_UPDATE"}})";

    ws_->async_write(net::buffer(subscription), [this](beast::error_code ec, std::size_t) {
        if (!ec) {
            std::cout << "Subscription sent!" << std::endl;
            ws_->async_read(buffer_, beast::bind_front_handler(&ExchangeWebsocketClient::on_read, this));
        }
    });
}

void ExchangeWebsocketClient::on_read(beast::error_code ec, std::size_t bytes_transferred) {
